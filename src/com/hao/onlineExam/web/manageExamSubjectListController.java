...
public class manageExamSubjectListController {
	
	@Autowired
	private IExamSubjectListService listService;
	
	@Autowired
	private IExamTestService testService;
	...
	public String show(Model model,HttpSession session) throws Exception {
		...
		List<SubjectVO> list = listService.findAllSubject(userVO);
	...
	public ExamScoreVO showScore(HttpSession session,Integer scoreId){
		ExamScoreVO vo = listService.getScoreById(scoreId);
		return vo;
	}
	...
	public String test(HttpSession session,Model model,@PathVariable Integer subId) throws Exception {
		model.addAttribute("testList", testService.getTestListBySubject(subId,
									((User)session.getAttribute("user")).getUserId()));
		model.addAttribute("subject", 
				subjectService.getSubjectVOById(((User)session.getAttribute("user")).getUserId(), subId));
		return "student/testDemo";
	}
	...
	public String assignMent(HttpServletRequest request,Integer subId,Model model,RedirectAttributes redirectAttributes){
		...
		if(entrySet != null){
			...
			ExamScoreVO score = testService.calculateScore(answerIds,testMap,((User)request.getSession().getAttribute("user")).getUserId(),subId);
			ExamSubject subjectVO = testService.getSubjectById(subId);
